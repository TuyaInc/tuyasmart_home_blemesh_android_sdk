...
import com.smart.tuya.meshdemo.activity.SigMeshControlActivity;
...
import com.smart.tuya.meshdemo.utils.DeviceTypeUtils;
...
public class SigMeshListPresenter implements IMeshListPresenter {
    ...
    public SigMeshListPresenter(Activity context, IMeshDeviceListView view, long homeId, String meshId) {
        ...
        mTuyaBlueMeshDevice = TuyaHomeSdk.newSigMeshDeviceInstance(meshId);
    ...
    public void startClient() {
        if (meshBean != null) {
            if (check()) {
                TuyaHomeSdk.getTuyaSigMeshClient().startClient(meshBean);
            }
        } else {
    ...
    private void showLightControlActivity(DeviceBean deviceBean, String devId) {
        ...
        switch (DeviceTypeUtils.getSigMeshProductType(vendorId)) {
            case "01":
                // 灯大类
                intent = new Intent(mContext, SigMeshControlActivity.class);
            default:
        }
    ...
    public void getStatusAll() {
        ...
        // sigmesh 暂时没有获取所有设备状态的的功能  可以通过遍历设备列表 逐个获取
